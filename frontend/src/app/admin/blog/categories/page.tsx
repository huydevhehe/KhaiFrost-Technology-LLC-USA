...
function toForm(category: AdminPostCategory | null): FormState {
  return {
    slug: category?.slug ?? "",
    nameVi: category?.vi?.name ?? "",
    nameEn: category?.en?.name ?? "",
    descriptionVi: category?.vi?.description ?? "",
...
export default function PostCategoriesPage() {
  ...
  const { categories, loading, error, reload } = usePostCategories();
  ...
  const canManage = hasPermission(PERMISSIONS.POST_CATEGORY_MANAGE);
  ...
  const startEdit = (category: AdminPostCategory) => {
  ...
  const submit = async () => {
    ...
    const slugError = validateSlug(form.slug, POST_CATEGORY_LIMITS.slug);
    if (slugError) {
    ...
    const result = await action.run(
      () =>
        editing
          ? postCategoriesApi.update(editing.id, {
    ...
    const ok = await confirm({
      ...
      message:
        category.postCount > 0
          ? `Danh mục đang có ${category.postCount} bài viết. Hãy chuyển các bài viết này sang danh mục khác trước.`
          : "Danh mục này sẽ bị xoá vĩnh viễn.",
      confirmLabel: "Xoá danh mục",
      danger: true,
    });
    if (!ok) return;
    const done = await action.run(() => postCategoriesApi.remove(category.id), {
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Danh mục bài viết</h1>
        </div>
        {canManage && (
      ...
      <Panel>
        {loading ? (
          ...
          <EmptyState
            ...
            description="Danh mục giúp nhóm các bài viết theo chủ đề."
            icon={<FolderCog size={28} />}
          />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <caption className="sr-only">Danh sách danh mục bài viết</caption>
              <thead>
                ...
                {categories.map((category) => (
                  <tr key={category.id} className="text-slate-700">
                    <td className="py-3 pr-4">
                      <p className="font-medium text-slate-900">{category.vi?.name ?? "—"}</p>
                      <p className="text-xs text-slate-400">{category.en?.name ?? "—"}</p>
                    </td>
                    <td className="py-3 pr-4 text-slate-500">/{category.slug}</td>
                    <td className="py-3 pr-4 text-slate-500">{category.postCount}</td>
                    <td className="py-3 pr-4">
                      <span
        ...
        >
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Tên (VI)" htmlFor="category-name-vi" required>
                <Input
                  id="category-name-vi"
                  value={form.nameVi}
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...
                      slug:
                        editing === null
                          ? slugify(event.target.value, POST_CATEGORY_LIMITS.slug)
                          : current.slug,
                    }))
                  }
                />
              </Field>
              <Field label="Tên (EN)" htmlFor="category-name-en" required>
                <Input
                  id="category-name-en"
                  value={form.nameEn}
                  maxLength={POST_CATEGORY_LIMITS.name}
              ...
              <Field label="Mô tả (VI)" htmlFor="category-desc-vi">
                <Textarea
                  id="category-desc-vi"
                  rows={3}
                  value={form.descriptionVi}
                  maxLength={POST_CATEGORY_LIMITS.description}
                  onChange={(event) =>
                    setForm((current) => ({ ...current, descriptionVi: event.target.value }))
                  }
                />
              </Field>
              <Field label="Mô tả (EN)" htmlFor="category-desc-en">
                <Textarea
                  id="category-desc-en"
                  rows={3}
              ...
              <Field label="Đường dẫn" htmlFor="category-slug">
                <Input
                  id="category-slug"
                  value={form.slug}
              ...
              <Field label="Thứ tự hiển thị" htmlFor="category-sort">
                <Input
                  id="category-sort"
                  type="number"
                  min={0}
